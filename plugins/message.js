...
global.messageDevs = message => {
    ...
    startupChannel = client.channels.resolve(startChannel);

    if (startupChannel) {
        return startupChannel.send(`<@${devs.join('>, <@')}> - ${message}`).catch(console.error);
    } else {
        console.error(`Could not send message to ${startChannel}`);
        console.log(message);
    }
}
